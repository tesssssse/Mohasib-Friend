...
async function uploadDocument(file, docName, row) {
    ...
    try {
        ...
        if (response.ok && data) {
            console.log(`تم رفع الملف بنجاح ${docName}`);
            ...
            createExpiryDateModal(docName, expiryDate, async (editedDate, isDateChanged) => {
                if (isDateChanged) {
                    ...
                    console.log('Expiry date not changed.');
                }
...
function updateTableWithStatus(fileDetails) {
    ...
    fileDetails.forEach((fileDetail) => {
        ...
        console.log('Processing file detail:', fileDetail);
        ...
        if (fileDetail.files && fileDetail.files.length > 0) {
            fileDetail.files.forEach(file => {
                ...
                console.log('Normalized folder name from response:', normalizedResponseName);
                ...
                let row = Array.from(tbody.children).find(row => {
                    ...
                    if (documentNameCell) {
                        ...
                        console.log('Comparing:', normalizedDocumentName, 'with:', normalizedResponseName);
...
async function fetchDocumentStatuses() {
    ...
    try {
        ...
        console.log('Fetched data after parsing:', data);
